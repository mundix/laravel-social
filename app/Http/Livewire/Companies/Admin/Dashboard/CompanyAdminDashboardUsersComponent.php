...
namespace App\Http\Livewire\Companies\Admin\Dashboard;
...
class CompanyAdminDashboardUsersComponent extends Component implements Filters
{
    ...
    {
        return view('livewire.companies.admin.dashboard.company-admin-dashboard-users-component', [
    ...
    private function getChartColumns($period, $amount): array
    {
        ...
        if ($period == 'day') {
            ...
            for ($i = $amount; $i >= 1; $i--) {
                ...
                $columns[] = [
                    ...
                    'value' => (new CompanyService)->getEmployeesUserCollection($this->company, $startOfDay, $endOfDay)->count()
                ];
            }
        }
        ...
        if ($period == 'month') {
            for ($i = $amount; $i >= 1; $i--) {
                ...
                $columns[] = [
                    'name' => $date->format('M'),
                    'value' => (new CompanyService)->getEmployeesUserCollection($this->company, $firstOfMonth, $endOfMonth)->count()
                ];
            }
        }
